...
@dataclass
class ContrastRepairSingleLine:
    ...
    def _save_tmp_gpt_results(self):
        with open(os.path.join(self._save_folder, "SL_tmp_results.json"), "w") as f:
            json.dump(self._results, f, indent=4)

        with open(os.path.join(self._save_folder, "SL_tmp_gpt_records.json"), "w") as f:
    ...
    def repair(self, bugs):
        ...
        for bug, bug_js in bugs.items():
            ...
            while tries < self._total_tries and not true_valid:
                ...
                if sys_config.dataset.startswith('quixbugs'):
                    ...
                    # judge whether current bug is a single-line one
                    if bug in ['FLATTEN', 'LCS_LENGTH', 'POWERSET', 'SUBSEQUENCES']:
                        break
                    tmp_prompt = self._prompt.get_initial_prompt4quix(bug_js['buggy'], select_pairs, "SL", bug_js)
                else:
                    tmp_prompt = self._prompt.get_initial_prompt(bug_js['buggy'], select_pairs, "SL", bug_js)
                ...
                if max_tokens > 4000:
                    ...
                    try:
                        while temp_tokens >= 4000 and count < 200:
                            ...
                            if sys_config.dataset.startswith('quixbugs'):
                                # judge the bugs are from Quix-Java or Quix-Python
                                tmp_token_str = self._prompt.get_initial_prompt4quix(bug_js['buggy'], select_pairs, "SL", bug_js)
                            else:
                                tmp_token_str = self._prompt.get_initial_prompt(bug_js['buggy'], select_pairs, "SL", bug_js)
                ...
                if sys_config.dataset.startswith('quixbugs'):
                    # judge the bugs are from Quix-Java or Quix-Python
                    curr_prompt = self._prompt.get_initial_prompt4quix(bug_js['buggy'], select_pairs, "SL", bug_js)
                else:
                    curr_prompt = self._prompt.get_initial_prompt(bug_js['buggy'], select_pairs, "SL", bug_js)
                ...
                while prompt_times < self._repeats:
                    ...
                    logger.info(answer)
                    func, last_answer = self._chatgpt_parser.chatgpt_parse(answer, '', bug_js['suffix'], bug_js['prefix'])
                    ...
                    if func != "":
                        # output = v['prefix'] + "\n" + func.strip() + "\n" + v['suffix']
                        if sys_config.dataset == 'quixbugs-python':
                            output = bug_js['prefix'] + "\n" + bug_js['leading_whitespace'] + func.strip() + "\n" + bug_js['suffix']
                        else:
                            output = bug_js['prefix'] + "\n" + func.strip() + "\n" + bug_js['suffix']
                        ...
                        if sys_config.dataset.startswith('quixbugs'):
                            # judge the bugs are from Quix-Java or Quix-Python
                            if sys_config.dataset == 'quixbugs-python':
                                infill_patch = bug_js['leading_whitespace'] + func.strip()
                            else:
                                infill_patch = func.strip()
                            curr_prompt = self._prompt.get_iterative_prompt4quix(output, select_pairs,
                                                                                 error_message, bug_js, "SL", infill_patch)
                        else:
                            curr_prompt = self._prompt.get_iterative_prompt(output, select_pairs, error_message, bug_js,
                                                                        type='SL', infill_patch=func.strip())
                                ...
                                while temp_tokens >= 4000 and count < 200:
                                    ...
                                    if sys_config.dataset.startswith('quixbugs'):
                                        ...
                                        if sys_config.dataset == 'quixbugs-python':
                                            ...
                                            infill_patch = func.strip()
                                        tmp_token_str = self._prompt.get_iterative_prompt4quix(output, select_pairs,
                                                                                             error_message, bug_js,
                                                                                             "SL", infill_patch)
                                    else:
                                        tmp_token_str = self._prompt.get_iterative_prompt(output, select_pairs, error_message, bug_js,
                                                                        type='SL', infill_patch=func.strip())
                                    tmp_json_tokens_dict = {'message': tmp_token_str}
                                    tmp_json_tokens_list = [tmp_json_tokens_dict]
                                    temp_tokens = self._num_tokens_from_messages(tmp_json_tokens_list)
        ...
        with open(os.path.join(self._save_folder, sys_config.dataset+"SL_final_results.json"), "w") as f:
            json.dump(self._results, f, indent=4)

        with open(os.path.join(self._save_folder, sys_config.dataset+"SL_gpt_records.json"), "w") as f:
            json.dump(self._gpt_record, f, indent=4)
    ...
    def plausible_to_correct(self, bugs):
        INFILL_TOKEN = ">>> [ INFILL ] <<<"
        max_tokens = 100
        with open(os.path.join(self._save_folder, "ContrastRepair20_SL_tmp_results_56.json"), "r") as f:
            plausible_fixes = json.load(f)
            # underlying assumption is that there should be on plausible fix
            # and is computed using chatgpt

        results = {}
        plausible_list = ['Cli-17.java', 'Cli-8.java', 'Codec-10.java', 'Codec-18.java']
        for bug, v in plausible_fixes.items():
            ...
            for patch in v:
                if patch['valid']:
                    ...
                    break
            if plausible_patch != "":
                prompt = self._prompt.INIT_CHATGPT_INFILL_LINE_PFC_PROMPT.format(
                ...
                while tries < 50:
                    ...
                    config = self._chatgpt_config(prev={}, message=prompt + "\n" + self._prompt.PFC_SUFFIX_LINE_PROMPT, use_last=False, temperature=self._config.temperature)
                    ...
                    func, pre_history = self._chatgpt_parser.chatgpt_parse(ret["choices"][0]['message']["content"], '', bug_js['suffix'],
                                                                           bug_js['prefix'])
                    ...
                    if func != "":
                        ...
                        if output.replace(" ", "") not in generations:
                            ...
                            if valid:
                                num_plausible += 1
                                prompt += self._prompt.PFC_ADD_PROMPT.format(num=num_plausible, fix_hunk=func.strip())
                        else:
            ...
            with open(os.path.join(self._save_folder, "ConR_plausible2correct_SL.json"), "w") as f:
                json.dump(results, f)