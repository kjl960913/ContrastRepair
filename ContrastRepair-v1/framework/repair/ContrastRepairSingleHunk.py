...
@dataclass
class ContrastRepairSingleHunk:
    ...
    def _save_tmp_gpt_results(self):
        with open(os.path.join(self._save_folder, "SH_tmp_results.json"), "w") as f:
            json.dump(self._results, f, indent=4)

        with open(os.path.join(self._save_folder, "SH_tmp_gpt_records.json"), "w") as f:
    ...
    def repair(self, bugs):
        ...
        for bug, bug_js in bugs.items():
            ...
            while tries < self._total_tries and not true_valid:
                ...
                if sys_config.dataset.startswith('quixbugs'):
                    ...
                    if bug in ['FLATTEN', 'LCS_LENGTH', 'POWERSET']:
                        break
                    tmp_prompt = self._prompt.get_initial_prompt4quix(bug_js['buggy'], select_pairs, "SH", bug_js)
                else:
                    tmp_prompt = self._prompt.get_initial_prompt(bug_js['buggy'], select_pairs, "SH", bug_js)
                ...
                if max_tokens > 4000:
                    ...
                    try:
                        while temp_tokens >= 4000 and count < 200:
                            ...
                            if sys_config.dataset.startswith('quixbugs'):
                                # judge the bugs are from Quix-Java or Quix-Python
                                tmp_token_str = self._prompt.get_initial_prompt4quix(bug_js['buggy'], select_pairs, "SH", bug_js)
                            else:
                                tmp_token_str = self._prompt.get_initial_prompt(bug_js['buggy'], select_pairs, "SH", bug_js)
                ...
                if sys_config.dataset.startswith('quixbugs'):
                    # judge the bugs are from Quix-Java or Quix-Python
                    curr_prompt = self._prompt.get_initial_prompt4quix(bug_js['buggy'], select_pairs, "SH", bug_js)
                else:
                    curr_prompt = self._prompt.get_initial_prompt(bug_js['buggy'], select_pairs, "SH", bug_js)
                        ...
                        if output not in generations:
                            ...
                            try:
                                if sys_config.dataset.startswith('quixbugs'):
                                    ...
                                    file_name = bug + "_{}.java".format(
                                        len(generations)) if sys_config.dataset == 'quixbugs-java' else bug + "_{}.python".format(
                                        len(generations))
                                    valid, error_message = self._validator._write_file4quix(patch_folder, output,
                                                                                            file_name, bug,
                                                                                            lang=type)
                                else:
        ...
        with open(os.path.join(self._save_folder, sys_config.dataset+"SH_final_results.json"), "w") as f:
            json.dump(self._results, f, indent=4)

        with open(os.path.join(self._save_folder, sys_config.dataset+"SH_gpt_records.json"), "w") as f:
            json.dump(self._gpt_record, f, indent=4)

    def plausible_to_correct(self, bugs):
        INFILL_TOKEN = ">>> [ INFILL ] <<<"
        max_tokens = 100
        with open(os.path.join(self._save_folder, "SH_final_results.json"), "r") as f:
            plausible_fixes = json.load(f)
            # underlying assumption is that there should be on plausible fix
            # and is computed using chatgpt

        results = {}
        plausible_list = ['Chart-10.java', 'Chart-13.java',
                          'Closure-10.java', 'Closure-104.java',
                          'Closure-107.java', 'Closure-109.java']
        for bug, v in plausible_fixes.items():
            ...
            for patch in v:
                if patch['valid']:
                    ...
                    break
            if plausible_patch != "":
                prompt = self._prompt.INIT_CHATGPT_INFILL_PFC_PROMPT.format(
                ...
                while tries < 35:
                    ...
                    config = self._chatgpt_config(prev={}, message=prompt + "\n" + self._prompt.PFC_SUFFIX_PROMPT, use_last=False, temperature=self._config.temperature)
            ...
            with open(os.path.join(self._save_folder, "ContrastRepair_plausible2correct_SH.json"), "w") as f:
                json.dump(results, f)